...
-- ============================================================================
-- CONTAS INSTAGRAM (usadas pelos criativos do Meta Ads)
-- ============================================================================

-- O agendador e os relatórios de social saíram do site (migraram pro Yeshua);
-- o que sobrou é o cadastro das contas, que o wizard de campanha usa pra montar
-- o criativo (`ig_user_id` + página do Facebook).
...
-- ============================================================================
-- SEGURANÇA — tentativas de login no /admin
-- ============================================================================

-- Uma linha por tentativa (sucesso ou falha). O bloqueio por brute force é
-- calculado em cima da janela recente de falhas — ver lib/db/login-attempts.ts.
-- Guarda o hash do e-mail tentado, não o e-mail em claro, pra não virar lista
-- de alvos caso o banco vaze.
CREATE TABLE IF NOT EXISTS login_attempts (
  id            INTEGER PRIMARY KEY AUTOINCREMENT,
  ip            TEXT NOT NULL,
  email_hash    TEXT,                      -- sha256 do e-mail normalizado
  success       INTEGER NOT NULL DEFAULT 0,
  user_agent    TEXT,
  created_at    TEXT NOT NULL DEFAULT (datetime('now'))
);
...
CREATE INDEX IF NOT EXISTS idx_login_attempts_email ON login_attempts(email_hash, created_at);
