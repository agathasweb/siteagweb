    ...
    <main id="main-content" role="main" className="bg-black min-h-screen">
      <div className="mx-auto max-w-4xl px-6 py-20 lg:px-8">
        ...
        <div className="prose prose-invert prose-lg max-w-none space-y-6 text-gray-300">
          ...
          <h2 className="text-2xl font-bold text-white mt-8">{t.cookieTypesHeading}</h2>
          <ul className="list-disc pl-6 space-y-2">
            {t.cookieTypes.map((c) => (
              <li key={c.label}><strong className="text-white">{c.label}</strong> {c.desc}</li>
            ))}
          </ul>
          <h2 className="text-2xl font-bold text-white mt-8">{t.managementHeading}</h2>
