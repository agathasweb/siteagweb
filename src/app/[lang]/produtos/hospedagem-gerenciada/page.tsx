...
export async function generateMetadata({ params }: PageProps<'/[lang]/produtos/hospedagem-gerenciada'>): Promise<Metadata> {
  ...
  return {
    ...
    alternates: {
      canonical: `${origin}/produtos/hospedagem-gerenciada`,
      languages: buildHreflangAlternates('/produtos/hospedagem-gerenciada'),
    },
  }
}
    ...
    <main id="main-content" role="main">
      <section className="relative overflow-hidden bg-black py-20 sm:py-32">
        ...
        <div className="relative mx-auto max-w-7xl px-6 lg:px-8 text-center">
          <h1 className="text-4xl font-bold tracking-tight text-white sm:text-6xl">{t.hero.titlePrefix} <span className="text-cyan-400">{t.hero.titleHighlight}</span></h1>
      ...
      <section className="py-24 bg-voyia-dark">
        <div className="mx-auto max-w-7xl px-6 lg:px-8">
          ...
          <div className="text-center mt-16">
            <Link href="/contato" className="inline-flex items-center bg-cyan-600 hover:bg-cyan-700 text-white px-8 py-4 rounded-lg font-semibold transition-colors text-lg">{t.cta}</Link>
          </div>
        </div>
      </section>
    </main>
